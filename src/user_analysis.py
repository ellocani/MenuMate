...
class UserAnalysis:
    ...
    def visualize_user_preferences(self, user_name, top_k=10):
        ...
        # 빈도에 따른 색상 설정 (빈도 높을수록 진한 색)
        def generate_color_palette(data, base_color):
            max_value = data.max()
            min_value = data.min()
            normalized_values = (data - min_value) / (max_value - min_value)
            return sns.light_palette(base_color, reverse=True, as_cmap=False, n_colors=len(data))
        ...
        sns.barplot(x=taste_profile_data.index, y=taste_profile_data.values, ax=axes[0], palette=taste_palette)
        ...
        sns.barplot(x=category_counts.index, y=category_counts.values, ax=axes[1], palette=category_palette)
